...
class user
{
    ...
    function db_save($state = '') {
        ...
        if (isset($config) && $config['database']['active']) {
            ...
            $q->execute([$this->id]);
            if (!$q->rowCount()) {
                $db->prepare('INSERT INTO ' . $config['database']['bot_table'] . ' (chat_id,state,bots,models) VALUES (?,?,?,?)')->execute([$this->id,$state,'',json_encode([0 => ['name' => 'Default','format' => '{LINKWB}','limit' => 40]])]);
                return true;
            } else {
